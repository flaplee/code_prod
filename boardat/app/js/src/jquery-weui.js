...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    ...
    var tpl = '<div class="weui-dialog">' +
                ...
                ( params.text ? '<div class="weui-dialog__bd '+ params.modify +'">'+params.text+'</div>' : '')+
  ...
  $.confirm = function(text, title, onOK, onCancel, modify) {
    var config;
    if (typeof text === 'object') {
      config = text
    } else {
  ...
  $.prompt = function(text, title, onOK, onCancel, input, maxlength, modify) {
    var config;
    if (typeof text === 'object') {
      config = text;
    } else {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="text" class="weui-input weui-prompt-input '+ config.modify.input +'" id="weui-prompt-input" maxlength="' + (config.maxlength || '') + '" value="' + (config.input || '') + '" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
      ...
      },
      {
        ...
        onClick: function() {
          ...
          if (!config.empty && (input === "" || input === null)) {
            if(config.modify.focus){
              modal.find('.weui-prompt-input').on('focus mousedown', function () {
                $(this).addClass(config.modify.focus);
              });
            }else{
      ...
      if(config.modify.focus){
        this.find('.weui-prompt-input').on('focus mousedown', function () {
          $(this).addClass(config.modify.focus);
        });
      }else{
...
+ function($) {
  ...
  var show = function(html, className, modify) {
    className = className || "";
    var mask = $("<div class='weui-mask_transparent'></div>").appendTo(document.body);

    var tpl = '<div class="weui-toast ' + className + ' ' + (modify ? modify : '') + '">' + html + '</div>';
    var dialog = $(tpl).appendTo(document.body);
  ...
  $.toast = function(text, style, modify, callback) {
    if(typeof style === "function") {
      callback = style;
    }
    var className, iconClassName = 'weui-icon-success-no-circle';
    var duration = ((modify && modify.duration) ? modify.duration : toastDefaults.duration);
    if(style == "cancel") {
    ...
    show('<i class="' + iconClassName + ' weui-icon_toast"></i><p class="weui-toast_content">' + (text || "已经完成") + '</p>', className, ((modify && modify.duration) ? modify.newname : undefined));
...
/* 
 * update 20180529 
 * 使用$.device.ios,处理touchstart,click
 *
 */
/* global $:true */
+ function($) {
  "use strict";
  $(document).on("click "+ (($.device.ios) ? 'touchstart' : '') +"", ".weui-search-bar__label", function(e) {
    $(e.target).parents(".weui-search-bar").addClass("weui-search-bar_focusing").find('input').focus();
  })
  
  .on("blur", ".weui-search-bar__input", function(e) {
    var $input = $(e.target);
    $input.val().length || ($input.val(""),$input.parents(".weui-search-bar").removeClass("weui-search-bar_focusing"));
  })
