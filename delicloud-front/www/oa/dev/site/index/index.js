...
define(['common/kernel/kernel', 'site/util/util'], function(kernel, util) {
    ...
    function orgdatachange(evt){
        ...
        if(index >= 0 && evt.data.organization && evt.data.organization.length > 0 && evt.data.organization[index]){
            $('a.nav-item-current .navlink-type').html(evt.data.organization[index].type == 'group' ? '<i class="iconfont icon-group">&#xe643;</i>' : '<i class="iconfont icon-user">&#xe642;</i>');
            $('a.nav-item-current .navlink-name').text(evt.data.organization[index].name);
            $('.nav-item-team .son-nav-wrap .son-nav-list-team').find('>').remove();
            //update 20180522
            evt.data.orgindex = index;
            if(evt.data.organization[evt.data.orgindex].id != evt.data.orgindexid){
                evt.data.orgindexid = evt.data.organization[evt.data.orgindex].id;
            }
            pagechange(evt);
            $.each(evt.data.organization, function(i, item) {
                ...
                var $targetHtml = $('<a class="sub-nav-item '+ ((evt.data.organization[index].id == item.id) ? 'current' : '') +'"  href="javascript:;" data-oid="' + item.id + '" data-pid="' + item.top_department_id + '">'+ targetIcon +'' + item.name + '</a>');
                ...
                return function(){
                    ...
                    $targetHtml.on('click', function(e) {
                        ...
                        if(!c.hasClass('current')){
                            $targetHtml.parents('.nav-item-team').find('a.nav-item-current .navlink-type').html('<i class="iconfont icon-group">&#xe643;</i>');
                            ...
                            util.setCookie('orgindexid', evt.data.organization[evt.data.orgindex].id);
    ...
    function pagechange(evt){
        ...
        util.setCookie('orgid', evt.data.organization[evt.data.orgindex].id),
        util.setCookie('parentid', evt.data.organization[evt.data.orgindex].top_department_id),
        util.setCookie('orgname', evt.data.organization[evt.data.orgindex].name),
        ...
        util.setCookie('employee_count', evt.data.organization[evt.data.orgindex].employee_cnt),
