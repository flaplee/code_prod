                    ...
                    case "imageChoose":
                        deli.common.image.choose({
                            ...
                            for(var i = 0; i < data.length; i++){
                                fileList.push(data[i].file_path)
                            }
...
","http://www.nbdeli.com/formwork/default/images/case-li-img.jpg","http://www.nbdeli.com/formwork/default/images/case-li-img4.jpg","http://www.nbdeli.com/formwork/default/images/case-li-img3.jpg"]
                    ...
                    case "navigationsetColors":
                        deli.common.navigation.setColors({
                            "color": "#5d85e0", //通知栏颜色及导航栏颜色
                            "notify_theme": "white", //通知栏主题 "white","black"
                            "nav_theme": "white" //导航栏主题  "white","black"
                        },function(data){
                        },function(resp){
                        });
                    break;
                    ...
                    case "notificationprompt":
                        var promptType = ["loading", "success", "error", "warning"],
                            testMsg = ["提示信息", "我是一条长度为14的提示信息","我是一条特别长的提示信息,此接口包含3个字段type、text、duration"];
                        deli.common.notification.prompt({
                            "type": promptType[Math.ceil(Math.random() * (promptType.length))-1], // loading, success, error, warning
                            "text": testMsg[Math.ceil(Math.random() * (testMsg.length))-1], //提示信息
                            "duration": 2 //提示时间,单位为秒
                        },function(data){
                        },function(resp){});
                    break;
                    case "modalshow":
                        var modalType = ["confirm", "alert", "prompt"],
                            titleMsg = ["标题", "我是一条长度为12的标题","我是一条特别长的标题,此接口包含3个字段type、title、content"],
                            contentMsg = ["内容", "我是一条长度为12的内容","我是一条特别长的内容,此接口包含3个字段type、title、content"];
                        deli.common.modal.show({
                            "type": modalType[Math.ceil(Math.random() * (modalType.length)) - 1], // confirm, alert, prompt
                            "title": titleMsg[Math.ceil(Math.random() * (titleMsg.length)) - 1], //提示的标题
                            "content": contentMsg[Math.ceil(Math.random() * (contentMsg.length)) - 1] //提示的内容
                        },function(data){
                            alert(JSON.stringify(data));
                        },function(resp){
                            alert(JSON.stringify(resp));
