...
function handleTencentVideo(link, callback, arg_obj) {
    ...
    function createPreviewVideoCallback(url,arg_obj) { 
        video_html='<iframe class="video_iframe" style="z-index:1;" height=' + arg_obj.height + ' width=' +arg_obj.width + ' frameborder=0 src="' + url + '" allowfullscreen></iframe>';
    }
    function convert_url(url, callback, arg_obj) {
        ...
        {
			return_url = url + '&width=' + arg_obj.width + '&height=' + arg_obj.height + '&auto=0';
            callback( return_url, arg_obj );
        }
    }
	...
	self.plugin.video = {
		edit : function() {
			var html = [
				...
				'目前支持学院内视频（直接复制视频URL）和<a href="http://v.qq.com/" target="_blank">腾讯视频</a>，请把视频上传到腾讯视频然后复制网站(v.qq.com开头的地址)到URL',
